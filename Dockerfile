...
# clone velora
RUN git clone https://github.com/velora-io/browser.git
WORKDIR /browser
...
RUN case $TARGETPLATFORM in \
    ...
    *) ARCH="x86_64" ;; \
    ...
    curl --fail -L -o libc_v8.a https://github.com/velora-io/zig-v8-fork/releases/download/${ZIG_V8}/libc_v8_${V8}_linux_${ARCH}.a && \
    mkdir -p v8/ && \
...
COPY --from=0 /browser/zig-out/bin/velora /bin/velora
COPY --from=1 /usr/bin/tini /usr/bin/tini

EXPOSE 9222/tcp

# Velora install only some signal handlers, and PID 1 doesn't have a default SIGTERM signal handler.
...
CMD ["/bin/velora", "serve", "--host", "0.0.0.0", "--port", "9222", "--log-level", "info"]
