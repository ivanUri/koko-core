...
// binding.cpp but missing from older lightpanda libc_v8.a prebuilts.
...
extern String *v8__String__NewFromUtf8(Isolate *isolate, const char *data, NewStringType type, int length);
extern int v8__String__Utf8Length(const String *self, Isolate *isolate);
extern int v8__String__WriteUtf8(const String *self, Isolate *isolate, char *buffer, int length, int options);
...
// Decode UTF-8 from WriteUtf8 into UTF-16 code units (lossy for invalid sequences).
void v8__String__WriteUtf16(const String *self, Isolate *isolate, uint32_t offset, uint32_t length, uint16_t *buffer) {
    (void)offset;
    int utf8_len = v8__String__Utf8Length(self, isolate);
    if (utf8_len <= 0 || length == 0) return;
    ...
    while (i < n && out < length) {
        ...
        if (c < 0x80) {
            ...
            cp = ((c & 0x0F) << 12) | (((unsigned char)tmp[i + 1] & 0x3F) << 6) | ((unsigned char)tmp[i + 2] & 0x3F);
            i += 3;
        } else if ((c & 0xF8) == 0xF0 && i + 3 < n) {
            cp = ((c & 0x07) << 18) | (((unsigned char)tmp[i + 1] & 0x3F) << 12) |
                 (((unsigned char)tmp[i + 2] & 0x3F) << 6) | ((unsigned char)tmp[i + 3] & 0x3F);
            i += 4;
        } else {
            cp = 0xFFFD;
            i += 1;
...
// Approximate: prebuilt lacks this C wrapper; current context is close enough for serve smoke tests.
