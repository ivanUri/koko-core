#!/usr/bin/env node
// Test Google reCAPTCHA v2 demo via Velora CDP + SDK.
//
// Usage:
//   node code-check/sites/recaptcha/demo.mjs
//   node code-check/sites/recaptcha/demo.mjs --endpoint http://127.0.0.1:9222 --keep
//   node code-check/sites/recaptcha/demo.mjs --timeout 90000 --wait 10 --poll 45
...
const TARGET_URL = "https://www.google.com/recaptcha/api2/demo";
...
function parseArgs(argv) {
    const out = {
        ...
        keep: false,
        output: resolve(repoRoot, "code-check/tmp/recaptcha-demo"),
        timeout: 90_000,
        waitSeconds: 10,
        pollSeconds: 45,
    };
    for (let i = 0; i < argv.length; i += 1) {
        ...
        switch (a) {
            ...
            case "--help":
                console.log("Usage: node recaptcha-demo-test.mjs [--endpoint <url>] [--keep] [--output <dir>] [--timeout <ms>] [--wait <s>] [--poll <s>]");
                process.exit(0);
                break;
            default:
...
async function pollCaptchaState(page) {
    return page.evaluate(`(() => {
        ...
        const widget = document.querySelector('.g-recaptcha, .g-recaptcha-badge');
...
async function main() {
    ...
    try {
        ...
        page.session.on("Runtime.consoleAPICalled", (event) => {
            const args = (event.args || []).map((a) => a.value ?? a.description ?? `<${a.type}>`);
        ...
        if (opts.waitSeconds > 0) {
            console.log(`[wait] ${opts.waitSeconds}s for reCAPTCHA scripts`);
        ...
        if (iframeBox.error) {
            ...
            // S1: coordinate click at iframe center (parent frame coords)
            try {
                const { x, y } = iframeBox.box;
                ...
                strategies.coordinateClick = `ok@${x.toFixed(1)},${y.toFixed(1)}`;
                console.log(`[click] S1 coordinate at (${x.toFixed(1)}, ${y.toFixed(1)})`);
            } catch (e) {
                strategies.coordinateClick = `error:${e?.message ?? e}`;
                console.log(`[click] S1 failed: ${e?.message ?? e}`);
            }
        }
        ...
        try {
            ...
            console.log(`[click] S2 DOM: ${strategies.domClick}`);
        } catch (e) {
            strategies.domClick = `error:${e?.message ?? e}`;
            console.log(`[click] S2 failed: ${e?.message ?? e}`);
        }

        // Poll for token / challenge
        ...
        // Network: recaptcha-related requests
        ...
        for (const r of failed) {
            if (/recaptcha|google/i.test(r.url || "")) {
                networkErrors.push({ url: r.url, failure: r.failureText });
            }
        }
        const badStatus = recaptchaReqs.filter((r) => (r.response?.status ?? 0) >= 400);
        for (const r of badStatus) {
            networkErrors.push({ url: r.url, status: r.response?.status });
        }

        console.log("\n=== reCAPTCHA network ===");
        for (const r of recaptchaReqs.slice(0, 15)) {
            const st = r.response?.status ?? (r.failureText ? `ERR:${r.failureText}` : "pending");
            console.log(`  [${st}] ${r.url?.slice(0, 100)}`);
        }
        ...
        const html = await page.content().catch((e) => `<!-- ${e} -->`);
        ...
        await writeFile(resolve(opts.output, "report.json"), JSON.stringify({
            ...
            console: consoleEntries.slice(-20),
        }, null, 2));
