...
ws.on("message", raw=>{
  ...
  if(m.method==="Network.requestWillBeSent"){
    ...
    net.push({t:'req', url:r.url.slice(0,160), method:r.method, post:(r.postData||'').slice(0,300)});
  }
  if(m.method==="Network.responseReceived"){
    const r=m.params.response;
    net.push({t:'res', url:r.url.slice(0,160), status:r.status});
  }
  if(m.method==="Runtime.exceptionThrown"){
    console.error("EXC", m.params?.exceptionDetails?.exception?.description?.slice(0,400));
  }
});
...
for (let i=0;i<15;i++){
  await delay(1000);
  const r=await call("Runtime.evaluate",{expression:`({title:document.title,html:document.documentElement.outerHTML.length,links:document.querySelectorAll('a[href]').length,aws:typeof AwsWafIntegration,cookie:document.cookie.includes('aws-waf'),url:location.href})`,returnByValue:true},sessionId);
