...
|------------|--------|--:|----------:|--------|
| Baseline empty jar | Koko serve, Cookie=0, h3 hop-1 | 4 | **0%** | all `knitsail_bootstrap` ~91 KB |
| X-Browser-Validation = Chrome live | `KOKO_X_BROWSER_VALIDATION=uemYFgH1pQp+…` | 3 | **0%** | still knitsail; wire shows override |
...
| HAR-shaped cold CH | `KOKO_COLD_FULL_CH=1` + fat XCD + 7871.115 | 3 | **0%** | Downlink/RTT/form-factors on wire; knitsail |
...
| Downlink/RTT + form-factors on cold | **ruled out** alone (`KOKO_COLD_FULL_CH`) |
...
1. **`KOKO_X_BROWSER_VALIDATION`** / **`KOKO_X_CLIENT_DATA`** env overrides in `XBrowser.zig`.
2. **`KOKO_COLD_FULL_CH=1`** in `HttpProfile.zig` — sei-hop CH shape on cold docs.
...
KOKO_X_BROWSER_VALIDATION='uemYFgH1pQp+sN1z7tIZXI0g3PI=' \
