...
| `velora.zig` | **Domain tuỳ biến riêng của Velora** (không thuộc chuẩn CDP) — mở rộng ngoài spec Chrome, đáng xem khi cần tính năng Velora-specific không có trong Playwright/Puppeteer chuẩn |
...
- Chạy qua **subcommand `velora mcp`** — theo `--help`: *"Starts an MCP (Model Context Protocol) server over stdio"* — tức là **transport là stdio, KHÔNG phải HTTP/WebSocket** như CDP (`velora serve`). Đây là điểm khác biệt quan trọng: MCP dùng cho tích hợp trực tiếp với client hỗ trợ MCP (qua stdin/stdout), CDP dùng cho tích hợp kiểu Playwright/Puppeteer (qua WebSocket).
...
## 4. `src/adapters/server/Server.zig` (699 dòng) — `velora serve`
...
| `--user-data-dir` | OS app data dir (macOS: `~/Library/Application Support/velora`) | Thư mục gốc kiểu Chrome user-data-dir |
...
| `--user-agent-suffix` | — | Thêm hậu tố vào User-Agent mặc định `Velora/X.Y` |
...
```
velora profile list
velora profile create --name <id> [--fingerprint <fingerprint-id>]
...
Không cờ riêng — dùng lại tập cờ chung (cookie, robots, block-private-networks, proxy, http-timeout...). Chạy `velora mcp` (không tham số) → server MCP qua stdio.
...
**Kết luận quan trọng**: đừng lầm tưởng `src/public/` là nơi Velora thực sự expose ra ngoài. API thật mà end-user/SDK dùng là **CDP** (`velora serve` → `@velora/sdk` ở repo `velora-sdk` riêng, kết nối `Browser.connect("http://127.0.0.1:9222")` theo README) và **CLI** (`velora fetch`). `src/public/` trông giống một lớp facade Rust/Zig-embedding dự định cho tương lai (dùng trực tiếp Velora như thư viện Zig) nhưng **chưa được hiện thực hoá** tại thời điểm khảo sát này — nếu cần kiểm tra lại tình trạng này, đọc trực tiếp cả 4 file (chỉ tốn vài giây, tổng 142 dòng).
...
| `crash_handler.zig` | Panic handler tuỳ biến, ghi đè handler mặc định Zig — in `"Velora has crashed..."` + `reason` + `OS`/`mode`/`version` + cố gắng dump stack trace (thường thất bại vì debug info bị strip — xem `build-and-dependencies.md` mục Zig 0.15.2 bug). Gọi `std.posix.abort()` cuối cùng → tiến trình nhận SIGABRT, **bắt được bằng `lldb`**. |
...
| `cli.zig` | `Builder` — framework parse CLI args dùng chung (không riêng cho velora, generic). |
