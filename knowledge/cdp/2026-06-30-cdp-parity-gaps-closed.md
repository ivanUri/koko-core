...
Velora's CDP surface had several no-op or blocking gaps that broke Playwright-shaped clients: viewport overrides did nothing, `LP.clickNode` could hang on human pointer animation, permissions and navigator emulation were not wired, and Network/Fetch events were incomplete. This change implements the missing Emulation state, fast LP activation, Browser permissions, navigator emulation overrides, Network extraInfo events, Input optional params, Fetch stale-intercept handling, DOM getContentQuads for document/text, isolated-world context recreation, and SDK `page.route()`.
...
| `LP.clickNode` + `HumanInput.movePointerTo` | CDP clients timed out on agent clicks |
...
- **`InputController.dispatchActivationOnElementFast`**: LP clicks skip bezier pointer moves.
