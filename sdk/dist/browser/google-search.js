...
export function buildGoogleExtractExpression(limit = 5) {
    return `(() => {
  const out = [];
  const seen = new Set();
...
export function validateGoogleExtract(data) {
    if (!data || typeof data !== "object")
        throw new ProtocolError("google search: invalid extract payload");
    if (data.pathHint?.blocked)
        throw new ProtocolError("google search: blocked (sorry/captcha/sign-in)");
    if (!data.resultCount || data.resultCount < 1) {
        throw new ProtocolError(`google search: no organic results (title=${data.title ?? "?"})`);
    }
}
export function buildGoogleSearchUrl(query, hl = "en") {
    const params = new URLSearchParams({ q: query, hl });
    return `https://www.google.com/search?${params}`;
}
export async function searchGoogle(page, options) {
