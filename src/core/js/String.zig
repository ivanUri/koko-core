...
pub fn toSSO(self: String, comptime global: bool) !(if (global) @import("../../support/string.zig").String.Global else @import("../../support/string.zig").String) {
    if (comptime global) {
        return .{ .str = try self.toSSOWithAlloc(self.local.ctx.page.frame_arena) };
    }
    return self.toSSOWithAlloc(self.local.call_arena);
}
pub fn toSSOWithAlloc(self: String, allocator: Allocator) !@import("../../support/string.zig").String {
    const handle = self.handle;
