...
fn dispatchSelectionChangeEvent(self: *Input, frame: *Frame) !void {
    const event = try Event.initTrusted(.wrap("selectionchange"), .{ .bubbles = true }, frame._page);
...
pub fn getSrc(self: *const Input, frame: *Frame) ![]const u8 {
    ...
    return @import("../../../browser/URL.zig").resolve(frame.call_arena, frame.base(), src, .{});
}
...
pub fn select(self: *Input, frame: *Frame) !void {
    ...
    const event = try Event.initTrusted(comptime .wrap("select"), .{ .bubbles = true }, frame._page);
    try frame._event_manager.dispatch(self.asElement().asEventTarget(), event);
}
