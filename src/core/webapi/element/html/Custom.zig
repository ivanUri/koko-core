...
pub fn invokeConnectedCallback(self: *Custom, frame: *Frame) void {
    if (comptime builtin.mode == .Debug) {
        std.debug.assert(frame.realmState() != .dead);
    }
...
pub fn invokeConnectedCallbackOnElement(comptime from_parser: bool, element: *Element, frame: *Frame) !void {
    ...
    // Parser-created elements can be synchronously upgraded before nodeComplete,
    // so all paths must respect the connected-callback state for this element.
...
fn invokeCallbackOnElement(element: *Element, definition: *CustomElementDefinition, comptime callback_name: [:0]const u8, args: anytype, frame: *Frame) void {
    ...
    frame.js.execution.validateJsEntry(.allow_draining, .custom_element) catch {
        RealmLifecycleKernel.traceCeCallback("ce.callback.drop_stale", frame._frame_id, frame.realmEpoch(), frame.realmState());
        return;
    };
...
pub fn attachBuiltInDefinition(element: *Element, definition: *CustomElementDefinition, frame: *Frame, comptime invoke_constructor: bool) !void {
    if (frame.getCustomizedBuiltInDefinition(element) != null) return;
    const extends_tag = definition.extends orelse return;
    if (extends_tag != element.getTag()) return;
    ...
    if (!comptime invoke_constructor) return;
    ...
    _ = local.toLocal(definition.constructor).newInstance(&caught) catch |err| {
        log.warn(.js, "custom builtin ctor", .{ .name = definition.name, .err = err, .caught = caught });
        return;
    };
}
...
pub fn checkAndAttachBuiltIn(element: *Element, frame: *Frame) !void {
    ...
    try attachBuiltInDefinition(element, definition, frame, true);
}
...
fn invokeCallback(self: *Custom, comptime callback_name: [:0]const u8, args: anytype, frame: *Frame) void {
    ...
    if (comptime std.mem.eql(u8, callback_name, "connectedCallback")) {
        RealmLifecycleKernel.traceCeCallback("ce.callback.connected", frame._frame_id, frame.realmEpoch(), frame.realmState());
    } else if (comptime std.mem.eql(u8, callback_name, "disconnectedCallback")) {
        RealmLifecycleKernel.traceCeCallback("ce.callback.disconnected", frame._frame_id, frame.realmEpoch(), frame.realmState());
    }
