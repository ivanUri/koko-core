...
fn dispatchEvent(self: *Media, comptime name: []const u8, frame: *Frame) !void {
    // Media events are generated by the user agent. The dispatch pipeline owns
    // their only transient reference and releases their arena after dispatch.
    const event = try Event.initTrusted(comptime .wrap(name), .{ .bubbles = false, .cancelable = false }, frame._page);
