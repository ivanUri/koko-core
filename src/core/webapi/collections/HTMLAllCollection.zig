...
pub fn length(self: *HTMLAllCollection, frame: *const Frame) u32 {
    ...
    assert(self._last_index == 0, "HTMLAllCollection.length", .{ .last_index = self._last_index });
...
pub const JsApi = struct {
    ...
    pub const @"[str]" = bridge.namedIndexed(HTMLAllCollection.getByName, null, null, null, .{ .null_as_undefined = true });
