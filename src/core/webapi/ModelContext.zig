...
pub fn registerTool(
    ...
    // AbortSignal dependents for model-context tools are Koko-specific; Koko's
    ...
    // CDP WebMCP notification hooks can be wired later without changing the
    // JS surface (`navigator.modelContext` / `window.modelContext`).
}
