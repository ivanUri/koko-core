...
pub fn init(url: []const u8, opts_: ?Opts, exec: *const Execution) !*EventSource {
    ...
    const self = try exec._factory.eventTargetWithAllocator(arena, EventSource{
        ...
        ._max_response_size = exec.context.page.session.browser.http_client.max_response_size,
    });
...
fn connect(self: *EventSource) !void {
    ...
    if (self._last_event_id.items.len > 0) {
        // headers.add copies the value, so the call arena's lifetime is enough
        const header = try std.fmt.allocPrintSentinel(exec.call_arena, "Last-Event-ID: {s}", .{self._last_event_id.items}, 0);
    ...
    if (!same_origin) {
        ...
        const origin = (URL.getOrigin(exec.call_arena, exec.url.*) catch null) orelse "null";
        const header = try std.fmt.allocPrintSentinel(exec.call_arena, "Origin: {s}", .{origin}, 0);
        try headers.add(header);
    }
    ...
    http_client.request(.{
        ...
        self.releaseRef(exec.context.page);
        return err;
    };
}

fn reconnectTask(self: *EventSource) void {
    if (self._ready_state != .connecting or self._http_response != null) {
...
fn scheduleTask(self: *EventSource, comptime task: fn (*EventSource) void, ms: u32, name: []const u8) !void {
    const Wrapper = struct {
        fn run(ctx: *anyopaque) anyerror!?u32 {
            ...
            defer es.releaseRef(es._exec.context.page);
            task(es);
            return null;
        }
        fn finalize(ctx: *anyopaque) void {
            ...
            es.deactivate();
            es.releaseRef(es._exec.context.page);
        }
    };

    self.acquireRef();
    errdefer self.releaseRef(self._exec.context.page);
...
fn httpStartCallback(response: HttpClient.Response) !void {
    const self: *EventSource = @ptrCast(@alignCast(response.ctx));
...
fn httpHeaderDoneCallback(response: HttpClient.Response) !bool {
    ...
    const status = response.status() orelse 0;
    const mime_ok = blk: {
        const ct = response.contentType() orelse break :blk false;
        const trimmed = std.mem.trim(u8, ct, &std.ascii.whitespace);
        break :blk std.ascii.startsWithIgnoreCase(trimmed, "text/event-stream");
    };
    ...
    if (!self._exec.isSameOrigin(self._url) and !self.corsAllowed(response)) {
        self.failConnection();
        return false;
    }

    const ctx = self._exec.context;
    const nested = ctx.local != null;
    var owned_scope: js.Local.Scope = undefined;
    if (!nested) ctx.localScope(&owned_scope);
    defer if (!nested) owned_scope.deinit();

    const final_url = try self._arena.dupeZ(u8, response.url());
...
// requests).
fn corsAllowed(self: *const EventSource, response: HttpClient.Response) bool {
    ...
    const origin = (URL.getOrigin(self._exec.call_arena, self._exec.url.*) catch null) orelse "null";
...
fn dispatchPending(self: *EventSource) !void {
    ...
    const event = try MessageEvent.initTrusted(typ, .{
        ...
        .last_event_id = self._last_event_id.items,
    }, exec.context.page);
...
fn dispatchEvent(self: *EventSource, comptime name: []const u8, handler: ?js.Function.Temp) !void {
    ...
    if (exec.hasDirectListeners(target, name, handler)) {
        const event = try Event.initTrusted(comptime .wrap(name), .{}, exec.context.page);
