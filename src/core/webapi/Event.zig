...
fn initWithTrusted(arena: Allocator, typ: String, opts_: ?Options, comptime trusted: bool) !*Event {
    ...
    const raw_timestamp = @import("../../support/datetime.zig").milliTimestamp(.monotonic);
    ...
    event.* = .{
        ...
        ._type_string = typ,
        ._is_trusted = trusted,
        // Internal trusted events are owned by the dispatch pipeline, not by
        // the creator callsite. They start with no Zig-side owner ref.
        ._rc = if (trusted) RC(u8).init(0) else RC(u8).init(1),
    };
    return event;
}
...
pub fn populatePrototypes(self: anytype, opts: anytype, trusted: bool) void {
    ...
    if (T == Event or @hasField(T, "is_trusted")) {
        ...
        // Trusted events created by browser runtime are short-lived and are
        // released by dispatch. Start with no creator-owned ref so lifecycle
        // does not depend on every callsite manually releasing.
        if (trusted) {
            self._rc = RC(u8).init(0);
        }
    }
}
