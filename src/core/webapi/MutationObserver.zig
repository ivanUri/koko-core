...
pub fn deliverRecords(self: *MutationObserver, frame: *Frame) !void {
    ...
    frame.js.execution.validateJsEntry(.allow_draining, .mutation_delivery) catch {
        RealmLifecycleKernel.traceMo("mo.drop_stale", frame._frame_id, null, frame.realmEpoch(), frame.realmState());
        self.clearPendingRecords(frame._page);
        return;
    };
