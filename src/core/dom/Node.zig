...
pub fn is(self: *Node, comptime T: type) ?*T {
    ...
    switch (self._type) {
        .element => |el| {
            ...
            if (comptime std.mem.indexOf(u8, type_name, ".webapi.element.") != null) {
                return el.is(T);
            }
        },
        .cdata => |cd| {
            ...
            if (comptime std.mem.indexOf(u8, type_name, ".webapi.cdata.") != null) {
                return cd.is(T);
            }
        },
        .attribute => |attr| {
        ...
        .document => |doc| {
            ...
            if (comptime std.mem.indexOf(u8, type_name, ".webapi.htmldocument.") != null) {
                return doc.is(T);
            }
        },
        .document_type => |dt| {
...
pub fn isEqualChildren(self: *Node, other: *Node) bool {
...
pub fn format(self: *Node, writer: *std.Io.Writer) !void {
    ...
    // return @import("../browser/dump.zig").deep(self, .{}, writer);
    return switch (self._type) {
...
pub fn jsonStringify(self: *const Node, writer: *std.json.Stringify) !void {
    ...
    return @import("../browser/dump.zig").toJSON(@constCast(self), writer);
}
