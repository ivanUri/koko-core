...
const timestampF = @import("../../../support/datetime.zig").timestamp;
...
fn setLifecycleEventsEnabled(cmd: *CDP.Command) !void {
    ...
    if (frame._load_state == .complete) {
        ...
        // HostIdle ledger (queues + intercept) — same as Runner.
        const HostIdle = @import("../../../core/browser/HostIdle.zig");
        const total_network_activity = HostIdle.totalHttpActivity(http_client);
...
fn close(cmd: *CDP.Command) !void {
    ...
    assert(bc.session.hasPage(), "CDP.frame.close null frame", .{});
...
fn createIsolatedWorld(cmd: *CDP.Command) !void {
    const params = (try cmd.params(struct {
        frameId: []const u8,
        worldName: []const u8,
        grantUniversalAccess: bool = false,
        grantUniveralAccess: bool = false,
    })) orelse return error.InvalidParams;
    const grant_universal_access = params.grantUniversalAccess or params.grantUniveralAccess;
    if (!grant_universal_access) {
        log.warn(.not_implemented, "Page.createIsolatedWorld", .{ .param = "grantUniversalAccess" });
        // When grantUniversalAccess == false and the client attempts to resolve
        // or otherwise access a DOM or other JS Object from another context that should fail.
    }
    const bc = cmd.browser_context orelse return error.BrowserContextNotLoaded;

    const world = try bc.createIsolatedWorld(params.worldName, grant_universal_access);
    const frame = bc.session.currentFrame() orelse return error.FrameNotLoaded;
...
pub fn frameChildFrameRemoved(bc: *CDP.BrowserContext, event: *const Notification.FrameChildFrameRemoved) !void {
    const session_id = bc.session_id orelse return;

    const frame_id = &id.toFrameId(event.frame_id);
    const parent_id = &id.toFrameId(event.parent_id);

    try bc.cdp.sendEvent("Page.frameDetached", .{ .params = .{
        .frameId = frame_id,
...
pub fn frameNavigated(arena: Allocator, bc: *CDP.BrowserContext, event: *const Notification.FrameNavigated) !void {
    ...
    if (!frame.realmReadyForExternalObservers()) {
        log.err(.cdp, "frameNavigated before realm ready", .{
    ...
    if (bc.scripts_on_new_document.items.len > 0) {
        ...
        for (bc.scripts_on_new_document.items) |script| {
            var try_catch: @import("../../../core/js/js.zig").TryCatch = undefined;
...
test "cdp.frame: captureScreenshot" {
    const LogFilter = @import("../../../testing/testing.zig").LogFilter;
