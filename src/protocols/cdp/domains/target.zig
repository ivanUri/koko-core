...
fn createTarget(cmd: *CDP.Command) !void {
    ...
    assert(!bc.session.hasPage(), "CDP.target.createTarget not null page", .{});

    // if target_id is null, we should never have a session_id
    assert(bc.session_id == null, "CDP.target.createTarget not null session_id", .{});
...
fn closeTarget(cmd: *CDP.Command) !void {
    ...
    assert(bc.session.hasPage(), "CDP.target.closeTarget null frame", .{});
...
fn sendMessageToTarget(cmd: *CDP.Command) !void {
    ...
    assert(bc.session_id != null, "CDP.target.sendMessageToTarget null session_id", .{});
