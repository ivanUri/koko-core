...
const FulfilledResponse = @import("../../../core/browser/HttpClient.zig").FulfilledResponse;
...
pub fn continueRequest(self: *InterceptionLayer, client: *Client, req: Request) anyerror!void {
    if (comptime IS_DEBUG) {
        assert(self.intercepted > 0, "InterceptionLayer.continueRequest", .{ .value = self.intercepted });
...
pub fn abortRequest(self: *InterceptionLayer, client: *Client, req: Request) void {
    if (comptime IS_DEBUG) {
        assert(self.intercepted > 0, "InterceptionLayer.abortRequest", .{ .value = self.intercepted });
...
pub fn fulfillRequest(
    ...
    if (comptime IS_DEBUG) {
        assert(self.intercepted > 0, "InterceptionLayer.fulfillRequest", .{ .value = self.intercepted });
