...
const EventListeners = struct {
    ...
    frame_child_frame_removed: List = .{},
...
const Events = union(enum) {
    ...
    frame_child_frame_removed: *const FrameChildFrameRemoved,
...
pub const FrameChildFrameRemoved = struct {
    frame_id: u32,
    parent_id: u32,
    timestamp: u64,
};
...
pub fn unregister(self: *Notification, comptime event: EventType, receiver: anytype) void {
    ...
    if (listeners.items.len == 0) {
        ...
        if (!removed) {
            log.err(.bug, "Notification.unregister", .{ .type = event });
