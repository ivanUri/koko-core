...
function injectProbes(html) {
    ...
    while ((m = re.exec(html))) {
        parts.push({ end: re.lastIndex, attrs: m[1] || "", body: m[2] || "" });
    }
    let inlineIdx = -1;
    let out = html;
    let offset = 0;
    for (const p of parts) {
        ...
        inlineIdx += 1;
        if (inlineIdx === 2 || inlineIdx === 3) {
            const probe = `<script>window.__after${inlineIdx}={kn:typeof globalThis.knitsail,sn:window.google?.sn,rs:document.readyState};</script>`;
...
async function main() {
    ...
    try {
        ...
        const res = await client.send("Runtime.evaluate", {
            expression: `({after2:window.__after2,after3:window.__after3,final:{kn:typeof globalThis.knitsail,sn:window.google?.sn,title:document.title}})`,
            returnByValue: true,
        }, sessionId);
